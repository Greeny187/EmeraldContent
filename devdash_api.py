# Dateien (Drop‑in‑Replacements)

---

## 1) `devdash_api.py` (Backend – AioHTTP)

```python
# devdash_api.py — Emerald DevDashboard v1.1
# Features added:
# - NEAR Connect (challenge + signature verify) + account binding to Telegram ID
# - Token Dynamics endpoints (NEP-141 read via RPC + off‑chain events)
# - Bot Mesh registry + health/metrics fan‑out
# - Hardened CORS + minor fixes
#
# Requirements (add to your runtime):
#   pip install psycopg-pool aiohttp httpx pynacl
# (pynacl is used for ed25519 signature verification)
...
async def me(request: web.Request):
    ...
    row = await fetchrow(
        "select username, role, tier, first_name, last_name, photo_url, near_account_id, near_public_key, near_connected_at from dashboard_users where telegram_id=%s",
        (user_id,),
    )
